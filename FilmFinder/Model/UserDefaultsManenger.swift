...
class DataManager: ObservableObject {
    ...
    @Published var history: [WatchedContent] = []  // Histórico de conteúdo assistido
    ...
    func loadFromUserDefaults() {
        ...
        if let savedFavorites = UserDefaults.standard.object(forKey: "favorites") as? Data,
           ...
           let savedHistory = UserDefaults.standard.object(forKey: "history") as? Data {
            if let loadedFavorites = try? decoder.decode([WatchedContent].self, from: savedFavorites),
               ...
               let loadedHistory = try? decoder.decode([WatchedContent].self, from: savedHistory) {
    ...
    func addToHistory(_ content: WatchedContent) {
        if !history.contains(where: { $0.content.idFilme == content.content.idFilme }) {
            history.append(content)
        }
    }

    func removeFromHistory(_ content: WatchedContent) {
        history.removeAll { $0.content.idFilme == content.content.idFilme }
    }
